 ...
 */
package com.yuntongxun.schoolgroup.storage;
...
public abstract class AbstractSQLManager {
    ...
    static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            if(oldVersion<18){
                onUpdateSessionTable(db);
                onUpdateIMTable(db);
            }
            
        }

        private void onUpdateSessionTable(SQLiteDatabase db) {

            String sql = "ALTER TABLE im_thread ADD  COLUMN isTop Integer";
            db.execSQL(sql);
        }
        private void onUpdateIMTable(SQLiteDatabase db) {

            String sql = "ALTER TABLE im_message ADD  COLUMN readcount Integer";
            db.execSQL(sql);
        }
        ...
        void createTableForIMessage(SQLiteDatabase db) {
            ...
            String sql = "CREATE TABLE IF NOT EXISTS "
                    ...
                    + IMessageColumn.READ_STATUS + "  INTEGER DEFAULT 0, "
                    + IMessageColumn.VERSION + "  INTEGER DEFAULT 0, "
                    + IMessageColumn.readCount + "  INTEGER DEFAULT 0, "
                    + IMessageColumn.DURATION + "  INTEGER DEFAULT 0"
                    + ")";
            LogUtil.v(TAG + ":" + sql);
            db.execSQL(sql);
        ...
        void createTableForISession(SQLiteDatabase db) {
            ...
            String sql = "CREATE TABLE IF NOT EXISTS "
                    ...
                    + IThreadColumn.IS_TOP + " INTEGER DEFAULT 0"
                    + ")";
            LogUtil.v(TAG + ":" + sql);
            db.execSQL(sql);
        }
